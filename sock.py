...
class Sock:
  ...
  def recebe_pacote(self, pack):
    ...
    print('TCP : INCOMING > ip origem : ',pack.ip_origem, \
          ...
          ' | conteudo : ', pack.conteudo.decode("UTF-8").strip('\n') if len(pack.conteudo) > 0 else '""')
    ...
    for packet in p:
      ...
      print('TCP: OUTGOING > ip origem : ',packet.ip_origem, \
            ...
            ' | FIN : ', flags[0], ' | SYN : ', flags[1],\
            ' | RST : ', flags[2], ' | ACK : ', flags[3],
            ' | conteudo : ', packet.conteudo.decode("UTF-8").strip('\n') if len(pack.conteudo) > 0 else '""')
