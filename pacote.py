...
def traduz_pseudo_cabecalho_ipv4(bin):
  ip_origem = endereco2str(bin[:4])
  ip_destino = endereco2str(bin[4:8])
  protocolo = bin[9]
  print(protocolo)
  assert protocolo == 6 # protocolo 6 é tcp
  segmento_tcp = bin[12:]
  return ip_origem, ip_destino, segmento_tcp
...
def traduz_pacote(binario):
  ip_origem, \
  ip_destino, \
  segmento_tcp = traduz_pseudo_cabecalho_ipv4(binario)
  porta_origem, \
  porta_destino, \
  numero_de_sequencia, \
  ...
  tamanho_da_janela, \
  checksum, \
  ponteiro_de_urgencia = struct.unpack('!HHIIHHHH', segmento_tcp[:20])
  tcp_offset = flags >> 12
